...
func RelayMidjourneyImage(c *gin.Context) {
	...
	if midjourneyTask == nil {
		c.JSON(400, gin.H{
			"error": "Midjourney task not found",
		})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Failed to HTTP GET image",
		})
		return
	}
	...
	if contentType == "" {
		// 如果None法confirm内容Type，则Default for jpeg
...
func RelayMidjourneyNotify(c *gin.Context) *dto.MidjourneyResponse {
	...
	if midjourneyTask == nil {
		return &dto.MidjourneyResponse{
			Code:        4,
			Description: "Midjourney task not found",
			Properties:  nil,
			Result:      "",
		}
	}
...
func RelaySwapFace(c *gin.Context) *dto.MidjourneyResponse {
	...
	// 如果没有 Configuration  price ，则使用Default price 
	if !success {
	...
	defer func(ctx context.Context) {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			...
			if quota != 0 {
				...
				logContent := fmt.Sprintf("Fixed Price for Model %.2f，Group rate %.2f，Operation %s", modelPrice, groupRatio, constant.MjActionSwapFace)
...
func RelayMidjourneyTaskImageSeed(c *gin.Context) *dto.MidjourneyResponse {
	...
	if channel.Status != common.ChannelStatusEnabled {
		return service.MidjourneyErrorWrapper(constant.MjRequestError, "The channel of this task has been disabled")
	}
...
func RelayMidjourneySubmit(c *gin.Context, relayMode int) *dto.MidjourneyResponse {
	...
	if relayMode == relayconstant.RelayModeMidjourneyAction { // midjourney plus，需要从customId中获取 Task 信息
	...
	if relayMode == relayconstant.RelayModeMidjourneyImagine { //绘画 Task ，此类 Task 可重复
	...
	} else if relayMode == relayconstant.RelayModeMidjourneyDescribe { //按Image to Text Task ，此类 Task 可重复
		midjRequest.Action = constant.MjActionDescribe
	} else if relayMode == relayconstant.RelayModeMidjourneyShorten { //缩短 Task ，此类 Task 可重复，plus only
		midjRequest.Action = constant.MjActionShorten
	} else if relayMode == relayconstant.RelayModeMidjourneyBlend { //绘画 Task ，此类 Task 可重复
		midjRequest.Action = constant.MjActionBlend
	} else if relayMode == relayconstant.RelayModeMidjourneyUpload { //绘画 Task ，此类 Task 可重复
		midjRequest.Action = constant.MjActionUpload
	} else if midjRequest.TaskId != "" { //Upscale、Variation Task ，此类 Task ，如果重复且已有 result ，远端api会直接Back最终 result 
		mjId := ""
		...
		if originTask == nil {
		...
		} else { //原 Task 的Status=SUCCESS，则可以做UpscaleUPSCALE、VariationVARIATION等动作，此时必须使用原来的请求地址才能正确处理
			...
			c.Set("channel_id", originTask.ChannelId)
			c.Request.Header.Set("Authorization", fmt.Sprintf("Bearer %s", channel.Key))
			log.Printf("检测到此Operation for Upscale、Variation、Reroll，获取原channel信息: %s,%s", strconv.Itoa(originTask.ChannelId), channel.GetBaseURL())
		}
	...
	defer func(ctx context.Context) {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			...
			if quota != 0 {
				tokenName := c.GetString("token_name")
				logContent := fmt.Sprintf("Fixed Price for Model %.2f，Group rate %.2f，Operation %s，ID %s", modelPrice, groupRatio, midjRequest.Action, midjResponse.Result)
				other := make(map[string]interface{})
				other["model_price"] = modelPrice
				other["group_ratio"] = groupRatio
				model.RecordConsumeLog(ctx, userId, channelId, 0, 0, modelName, tokenName, quota, logContent, tokenId, userQuota, 0, false, other)
	...
	//1-SubmitSuccess
	// 21-Task already exists（处理中或者有 result 了） {"code":21,"description":"Task already exists","result":"0741798445574458","properties":{"status":"SUCCESS","imageUrl":"https://xxxx"}}
	// 22-In Queue {"code":22,"description":"In Queue，前面还有1   Task ","result":"0741798445574458","properties":{"numberOfQueues":1,"discordInstanceId":"1118138338562560102"}}
	...
	// other: Submit错误，description for 错误描述
	midjourneyTask := &model.Midjourney{
		UserId:      userId,
		Code:        midjResponse.Code,
		Action:      midjRequest.Action,
	...
	if midjResponse.Code == 3 {
		//None实例账号Automatically DisabledChannel（No available account instance）
	...
	if midjResponse.Code != 1 && midjResponse.Code != 21 && midjResponse.Code != 22 {
		//非1-SubmitSuccess,21-Task already exists和22-In Queue，则记录错误 reason 
		midjourneyTask.FailReason = midjResponse.Description
		consumeQuota = false
	}

	if midjResponse.Code == 21 { //21-Task already exists（处理中或者有 result 了）
		// 将 properties 转换 for 一   map
	...
	if midjResponse.Code == 22 { //22-In Queue，说明Task already exists
		//修改Back值
